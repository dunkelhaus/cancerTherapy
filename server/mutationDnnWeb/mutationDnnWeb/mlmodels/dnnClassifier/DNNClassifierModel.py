...
class DNNClassifierModel:
    ...
    def learningRateCallback(args):

    def activationCallback(args):

    def regularizationCallback(args):

    def regularizationRateCallback(args):

    def problemTypeCallback(args):

    def batchSizeCallback(args):

    def noiseCallback(args):

    def trainToTestRatioCallback(args):

    def numHiddenLayersCallback(args):

    def networkShapeCallback(args):

    def resetCallback(args):

    def playCallback(args):

    def pauseCallback(args):

    def nextButtonCallback(args):

    def showTestDataCallback(args):

    def discretizeCallback(args):

    def featuresCallback(args):

    def datasetCallback(args):

    def weightsCallback(args):
