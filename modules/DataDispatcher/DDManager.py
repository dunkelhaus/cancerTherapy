...
class DDManager():
    ...
    def DispatchData(self,trainPaths,testPaths,dataDispatcherQ):
        print "Entered DDManager.DispatchData() with process: ", os.getpid()
        ...
        while((trainPaths.empty()==True and testPaths.empty()==True) or trainPaths.empty()==True or testPaths.empty()==True):
            ...
            print "Waiting for CVManager to generate train and test files..."
            time.sleep(1)
        ...
        print "Exiting DDManager.DispatchData() with process: ", os.getpid()
